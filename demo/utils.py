...
def gen_ca_bundle(ccf_crt_paths=None, 
                    ca_bundle_path=None, 
                    timeserver_ca_url="https://letsencrypt.org/certs/lets-encrypt-r3.pem"):
    if ccf_crt_paths is None:  
        # doing this here to prevent dependency of constants.py
        from constants import CCF_CERTS_DIR
        files = os.listdir(CCF_CERTS_DIR)
        print(CCF_CERTS_DIR, files)
        ccf_crt_paths = [os.path.join(CCF_CERTS_DIR, f) for f in files if "nodecert" in f]
    else:
        ccf_crt_paths = json.loads(ccf_crt_paths)

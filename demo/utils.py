...
def gen_ca_bundle(ca_bundle_path=None,
    ...
    timeserver_ca_buff = ""
    if timeserver_ca_urls is not None:
        if type(timeserver_ca_urls) != list:
            timeserver_ca_urls = [timeserver_ca_urls]
        for u in timeserver_ca_urls:
            response = requests.get(u)
            timeserver_ca_buff += response.content.decode("utf-8")
    else:
        if timeserver_ca_path is None:
            timeserver_ca_path = "/etc/ssl/certs/ca-certificates.crt"

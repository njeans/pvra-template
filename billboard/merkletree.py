...
def print_tree(nodes, index=-1, level=0):
    ...
    if left_index >= 0:
        output += print_tree(nodes, left_index, level+1)
        output += str("\t\t" * level) + '-> (0x' + node.hex()[:4] + "..)" + "\n"
        # output += str("\t\t" * level) + '-> (0x' + str(node) + "..)" + "\n"
        output += print_tree(nodes, right_index, level+1)
    else:
        output += str("\t\t" * level) + '-> {' + str(node) + "}" + "\n"
    return output

...
//    if (!d2i_X509(&cert, &p, sizeof(default_ca_bundle))) {
    ...
    if(DEBUGPRINT) printf("[nts] create_socket set fd\n");
    err = SSL_connect(ssl_session);
    printf("aaa\n");
    // err = sgxssl_connect(ssl_session);
    if (err != 1){
...
int create_socket(char* server_name, char* server_port)
{
    ...
    hints.ai_family = AF_UNSPEC; //todo check
    hints.ai_socktype = SOCK_STREAM;
    hints.ai_flags = 0;  //todo check
    hints.ai_protocol = IPPROTO_TCP; //todo check

    printf("[timestamp] call getaddrinfo\n");
    err = getaddrinfo(server_name, server_port, &hints, &result);
    if (err != 0) {
        printf("[timestamp] getaddrinfo failed\n");
        gai_strerror_print(err);
        return -1;
    }
    printf("result ai_family %d ai_socktype %d ai_protocol %d\n",result->ai_family, result->ai_socktype, result->ai_protocol);

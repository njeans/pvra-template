...
struct cResponse addPersonalData(struct ES *enclave_state, struct cInputs *CI, uint32_t uidx)
{
    ...
    ret.error = 0;
    if(uidx >= NUM_USERS) {
        sprintf(ret.message, "[apPVRA] invalid uid %d > %d", uidx, NUM_USERS);
        printf("%s\n", ret.message);
        ret.error = 1;
        return ret;

    }
    printf("[sdt] before uidx %d rc %d rt %d st %d\n", uidx, enclave_state->appdata.user_info[uidx].retrieve_count, enclave_state->appdata.user_info[uidx].retrieve_time, enclave_state->appdata.user_info[uidx].started_retrieve);
    ...
    printf("secret data %d: ", uidx);
    print_hexstring(enclave_state->appdata.user_info[uidx].secret_data, DATA_SIZE);
...
struct cResponse startRetrieve(struct ES *enclave_state, struct cInputs *CI, uint32_t uidx)
{
    ...
    printf("[sdt] startRetrieve uidx %d rc %d rt %d st %d\n", user_idx, enclave_state->appdata.user_info[user_idx].retrieve_count,enclave_state->appdata.user_info[user_idx].retrieve_time,enclave_state->appdata.user_info[user_idx].started_retrieve);
    ...
    printf("[sdt] success startRetrieve global rt %d uidx %d rc %d rt %d st %d\n", enclave_state->appdata.retrieve_count, user_idx, enclave_state->appdata.user_info[user_idx].retrieve_count,enclave_state->appdata.user_info[user_idx].retrieve_time,enclave_state->appdata.user_info[user_idx].started_retrieve);
...
struct cResponse completeRetrieve(struct ES *enclave_state, struct cInputs *CI, uint32_t uidx)
{
    ...
    for(int i = 1; i < NUM_USERS+1; i++) {
      printf("comparing\n");
      print_hexstring_n(&enclave_state->auditmetadata.master_user_pubkeys[i], 3);
      printf("..");
      print_hexstring_n(((char *)&enclave_state->auditmetadata.master_user_pubkeys[i])+61, 3);
      printf("\n");
        if(strncmp(&CI->user_pubkey, &enclave_state->auditmetadata.master_user_pubkeys[i], sizeof(secp256k1_pubkey)) == 0) {
          user_idx = i-1;
          break;
    ...
    print_hexstring(&enclave_state->auditmetadata.master_user_pubkeys[user_idx+1], KEY_SIZE);
...
//        memcpy_big_uint32(out[i], info.retrieve_count) todo
        printf("uidx %d: rc %d rt %d sr %d uidx %d ", i, enclave_state->appdata.user_info[i].retrieve_count, enclave_state->appdata.user_info[i].retrieve_time, enclave_state->appdata.user_info[i].started_retrieve, i);

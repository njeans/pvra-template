...
sgx_status_t ecall_commandPVRA(
  ...
  for(int i = 0; i < NUM_USERS+1; i++) {
    printf("comparing ");
    if(C_DEBUGPRINT) print_hexstring(&enclave_state.auditmetadata.master_user_pubkeys[i], 3);
...
//  if(C_DEBUGPRINT) print_hexstring(&enclave_state.enclavekeys.enc_prikey, 32);
//  if(C_DEBUGPRINT) print_hexstring(&user_pubkey, 32);

...
if [ "$2" != "omit_sig" ]; then
  ...
  echo "[bcPVRA] omitting data but returning signature"
  SIG="SIG"
  MSG="MSG"
fi

python3 $PROJECT_ROOT/billboard/billboard.py admin_sign_confirmation command.bin sig_admin.hexstring msg_admin.hexstring
